                ...
                <el-collapse>
                    ...
                    <el-collapse-item title="关系绑定" name="1">
                        ...
                        <div class='unlock-apply-container-form-select'>
                            ...
                            <el-select v-model='MediaIndex' @change="GetMediaMsg">
                                <el-option :label="`${item.civilno} 债事人: ${item.personName}`" :value="item.value" v-for='(item,index) in MediaSrc' :key='index'></el-option>
                            </el-select>
                        </div>
                        <h3>债事人信息</h3>
                        ...
                        <div class='unlock-apply-container-form-defuse-2'>
                            ...
                            <div>
                                本次申请转让债权金额（大写）：<input type="text" :value='SubmitData.amountThis|Uppercase'>
                            </div>
                            <div>
                            </div>
                        </div>
                    ...
                    <el-collapse-item title="咨询服务协议" name="3">
                        ...
                        <div class='unlock-apply-container-form-serve-agreement-commitment-3'>
                            ...
                            <div>
                                本次债权处理申请金额（大写）：<input type="text" :value='SubmitData.amountThis | Uppercase'>
                            </div>
                            <div>
                            </div>
                        </div>
                        ...
                        <div class='unlock-apply-container-form-serve-agreement-service-charge-1'>
                            ...
                            <div>
                                本次策划方案服务费（大写）：<input type="text" :value='SubmitData.servicePrincipal | Uppercase'>
                            </div>
                        </div>
                        <h3>其他约定事项</h3>
                            ...
                            <div class='unlock-apply-container-form-plan-agreement-sign'>
                                <div class='unlock-apply-container-form-plan-agreement-sign-left'>
                                    ...
                                    <div>
                                        甲方(签字盖章捺印)：
                                        <button>上传电子章</button>
                                        <input type="file" ref='PartASeal' class='unlock-apply-container-form-plan-agreement-sign-left-seal'>
                                    </div>
                                    <div>签约日期： <input type="text" v-model='SubmitData.partyaDate'></div>
                                ...
                                <div class='unlock-apply-container-form-plan-agreement-sign-right'>
                                    ...
                                    <div>签约日期： <input type="text" v-model='SubmitData.partybDate' placeholder="xxxx-xx-xx"></div>
...
export default {
            ...
            UnlockApplyMsg: [
                {
                    ...
                    ServeId: '1213das1211221',
                    ServeNumber: '12312dffasa',
                    Debtor: '莫莫',
    ...
    methods: {
        ...
        async UpdateFile (file) {
            ...
            return 'http://113.204.236.218:8989/' + result.split('/')[3]
        },
        UpdatePartASeal () {
        ...
        ViewContract () {
            this.$emit('onChangeFragment', 'PledgedContract')
        },
        // 解债申请信息初始化
        async InitData () {
            const formData = new FormData()
            formData.append('debtId', '7991157936871374848')
            const { data: result } = await this.$http({
                method: 'post',
            ...
            // 获取civilId
            const CivilMsgformData = new FormData()
            CivilMsgformData.append('civilId', '7992006815477415936')
            const { data: CivilMsgresult } = await this.$http({
                method: 'post',
                url: '/api/api/busCivilController/selectByPrimaryKey',
                data: CivilMsgformData,
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
            console.log(CivilMsgresult)
            // 获取relativePerId查询用户信息债事人,相对人信息
            const UserMsgformData = new FormData()
            UserMsgformData.append('relativePerId', CivilMsgresult.data.relativePerId)
            const { data: UserMsgresult } = await this.$http({
                method: 'post',
                url: '/api/api/busRelativePersonController/selectByRelativePerId',
                data: UserMsgformData,
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
        ...
        async SubmitMessage () {
            ...
            if (result.resultCode === '200') {
                ...
                const { data: StageUpdateResult } = await this.$http({
                    method: 'post',
                ...
                if (StageUpdateResult.resultCode === '200' && StatusUpdateResult.resultCode === '200') {
        ...
        GoInvestigationReport (index) {
            window.sessionStorage.setItem('debtId', 'debtId')
            this.$emit('onChangeFragment', 'ExamineReportForm')
        },
        // 查询担保人信息
        async Getguarantor () {
            const formData = new FormData()
            formData.append('civilId', '7992006815477415936') // 修改
            const { data: result } = await this.$http({
                method: 'post',
