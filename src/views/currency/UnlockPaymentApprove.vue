...
export default {
    ...
    methods: {
        async InitData () {
            ...
            formData.append('payId', window.sessionStorage.getItem('payId'))
        ...
        async PassCheck () {
            ...
            await this.UpdateCheckStatus('9').then(() => {
                // 通过解债ID查询信息
                const formData = new FormData()
                const debtId = window.sessionStorage.getItem('debtId')
                console.log(debtId)
                ...
                this.$http({
                    ...
                    this.AddPropertyMsg.debtId = window.sessionStorage.getItem('debtId')
