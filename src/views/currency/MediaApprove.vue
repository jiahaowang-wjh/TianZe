                ...
                <el-collapse>
                    ...
                    <el-collapse-item title="调解资料" name="1">
                        ...
                        <div class='civil-media-container-form-table-2'>
                            <div class='civil-media-container-form-table-2-item' v-for='(item,index) in GuarantorMsg' :key='index'>
                                <span>{{index+1}}</span>
                                <span>{{item.Type}}</span>
                        ...
                        <div>债券债务类型:
                            <label for="1">
                                <input type="radio" name="debt-type" id='1' :value="MediaData.civilType">借款
                            </label>
                            <label for="2">
                                <input type="radio" name="debt-type" id='2' :value="MediaData.civilType">贷款
                            </label>
                            <label for="3">
                                <input type="radio" name="debt-type" id='3' :value="MediaData.civilType">工程款
                            </label>
                            <label for="4">
                                <input type="radio" name="debt-type" id='4' :value="MediaData.civilType">其他款项
                            </label>
                        </div>
                        <h3>乙方尚欠甲方款项金额：</h3>
                        <!-- 欠款金额 -->
                        <div class='civil-media-container-form-own-amount'>
                            <div class='civil-media-container-form-own-amount-item-1'>
                                <span>欠款总额（小写）：<input type="text" :disabled='true' v-model='MediaData.amountTotal'></span>
                                <span>欠款总额（大写）：<input type="text" :disabled='true' :value='MediaData.amountTotal | Uppercase'></span>
                            </div>
                            <div class='civil-media-container-form-own-amount-item'>
                                <span>欠款总额中包含本金（小写）：<input type="text" :disabled='true' v-model='MediaData.amountPrincipal'></span>
                                <span>欠款总额中包含本金（大写）：<input type="text"  :disabled='true' :value='MediaData.amountPrincipal | Uppercase'></span>
                            </div>
                            <div>
                                <span>欠款总额中包含利息（小写）：<input type="text" :disabled='true' v-model='MediaData.amountInterest'></span>
                                <span>欠款总额中包含利息（大写）：<input type="text" :disabled='true' :value='MediaData.amountInterest | Uppercase'></span>
                            </div>
                            <div>
                                <span>欠款总额中包含违约（小写）：<input type="text" :disabled='true' v-model='MediaData.amountBreach'></span>
                                <span>欠款总额中包含违约（大写）：<input type="text" :disabled='true' :value='MediaData.amountBreach | Uppercase'></span>
                            </div>
                        </div>
                        <!-- 支付利息 -->
                        <h3>乙方向甲方支付款利息：</h3>
                        <div class='civil-media-container-form-interest'>
                            <div class='civil-media-container-form-interest-row1'>
                                <span>约定月利率标准：<input type="text" :disabled='true' v-model='MediaData.monthInterest'> </span>
                                <span>时间段：
                                    <el-date-picker :disabled='true' align="left" type="date" placeholder="请选择开始日期" :picker-options="pickerOptions" v-model="MediaData.starDate" value-format="yyyy-MM-dd hh-mm-ss"></el-date-picker>
                                    <span class='civil-media-container-form-interest-row1-separator'>—</span>
                                    <el-date-picker :disabled='true' align="left" type="date" placeholder="请选择结束日期"  v-model="MediaData.endDate"  :picker-options="pickerOptions1" value-format="yyyy-MM-dd hh-mm-ss"></el-date-picker>
                                </span>
                                <span>利息：<input type="text" :disabled='true' v-model='MediaData.interest'> </span>
                            </div>
                            <div class='civil-media-container-form-interest-row2'>
                                <span>已支付利息：<input type="text" :disabled='true' v-model='MediaData.interestPayment'></span>
                                <span>违约截止时间：<el-date-picker  :disabled='true' align="left" type="date" placeholder="请选择结束日期"  v-model="MediaData.breachDate" :picker-options="pickerOptions1" value-format="yyyy-MM-dd hh-mm-ss"></el-date-picker></span>
                                <span>已支付违约金额：<input type="text" :disabled='true' v-model='MediaData.breachMoney'></span>
                            </div>
                        </div>
                        <!-- 担保人担保方式： -->
                        <div>
                            担保人担保方式：
                            <label for="1">
                                <input type="radio" name="debt-type" id='1' value='1' v-model="MediaData.guaranteeMeth">保证
                            </label>
                            <label for="2">
                                <input type="radio" name="debt-type" id='2' value='2' v-model="MediaData.guaranteeMeth">质押
                            </label>
                            <label for="3">
                                <input type="radio" name="debt-type" id='3' value='3' v-model="MediaData.guaranteeMeth">留置
                            </label>
                            <label for="4">
                                <input type="radio" name="debt-type" id='4' value='4' v-model="MediaData.guaranteeMeth">定金
                            </label>
                        </div>
                        <h3>甲方、乙方、担保人经协商一致约定信息</h3>
                        <div class='civil-media-container-form-appoint'>
                            <div class='civil-media-container-form-appoint-row1'>
                                <span>确认债权债务本息金额合计（小写）：<input type="text" :disabled='true' v-model='MediaData.interestAll'></span>
                                <span>确认债权债务本息金额合计（大写）：<input type="text" :disabled='true' :value='MediaData.interestAll | Uppercase'></span>
                            </div>
                            <div class='civil-media-container-form-appoint-row2'>
                                <span>债事人自愿放弃的内容：<input type="text" :disabled='true' v-model='MediaData.abandonContent'></span>
                                <span>债务处理方式：
                                    <input type="text" :disabled='true' :value="MediaData.treatmentMethod === '1' ? '共同委托第三方处理' : ''">
                                </span>
                                <span></span>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        async InitData () {
            ...
            // 民事调解信息初始化
            const civilId = window.sessionStorage.getItem('civilId')
            const MsgFormData = new FormData()
            MsgFormData.append('civilId', civilId)
        ...
        async UpdateCheckStatus (status) {
            const formData = new FormData()
            this.CommitApproveData.civilId = window.sessionStorage.getItem('civilId')
