  ...
  <div class="roles">
    ...
    <div class="roles-list">
      <div class="roles-list-button">
        <el-button class="roles-list-button-new" @click="AddRole"
          >新增角色</el-button
        >
      </div>
      <div class="roles-list-content">
        ...
        <div class="roles-list-content-tab">
          <div
            class="roles-list-content-tab-item"
            v-for="(item, index) in RolesMsg"
            :key="index"
          >
            <span>{{ index + 1 }}</span>
            ...
            <span>{{ item.note }}</span>
            <span>
              <button type="button" @click="ShowDistributionPage(index)">
                分配权限
              </button>
              <button type="button" @click="OpenAlterPage(index)">编辑</button>
              <button type="button" @click="DeleteRole(index)">删除</button>
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="roles-add-dictionary" v-if="IsShowDistributionPage">
      <div class="roles-add-dictionary-box">
          ...
          <div class="roles-add-dictionary-box-body-menuList">
            ...
            <el-select multiple v-model="HasSelectMsg.MenuMsg">
              <el-option
                :label="item.menuName"
                :value="item.menuId"
                v-for="(item, index) in MenuMsgList"
                :key="index"
              ></el-option>
            </el-select>
            <el-select
              multiple
              v-model="HasSelectMsg.UserRightMsg"
              placeholder="请选择用户权限"
            >
              <el-option
                :label="item.interfacename"
                :value="item.authId"
                v-for="(item, index1) in UserRightMsgList"
                :key="index1"
              ></el-option>
            </el-select>
            <el-select
              multiple
              v-model="HasSelectMsg.DebtRightMsg"
              placeholder="请选择债务系统权限"
            >
              <el-option
                :label="item.interfacename"
                :value="item.authId"
                v-for="(item, index2) in DebtRightMsgList"
                :key="index2"
              ></el-option>
            </el-select>
            <el-select
              multiple
              v-model="HasSelectMsg.AssetsRightMsgList"
              placeholder="请选择资产系统权限"
            >
              <el-option
                :label="item.interfacename"
                :value="item.authId"
                v-for="(item, index3) in AssetsRightMsgList"
                :key="index3"
              ></el-option>
            </el-select>
          </div>
        ...
        <div class="roles-add-dictionary-box-footer">
          <button type="button" @click="CloseDistributionPage">取消</button>
          <button type="button" @click="SubmitAllDistributionData">确认</button>
        </div>
      </div>
    </div>
    <!-- 是否显示新增角色页面 -->
    <div class="roles-add-person" v-if="IsShowAddRole">
      <div class="roles-add-person-box">
        ...
        <div class="roles-add-person-box-body">
          ...
          <div>备注信息：<input type="text" v-model="AddRoleList.note" /></div>
        </div>
        <div class="roles-add-person-box-footer">
          <button type="button" @click="CloseAddPage">取消</button>
          <button type="button" @click="SubmitAddData">确认</button>
        </div>
      </div>
    </div>
    <!-- 是否显示新增角色页面 -->
    <div class="roles-add-person" v-if="IsShowAlterRole">
      <div class="roles-add-person-box">
        ...
        <div class="roles-add-person-box-body">
          ...
          <div>备注信息：<input type="text" v-model="AlterRoleMsg.note" /></div>
        </div>
        <div class="roles-add-person-box-footer">
          <button type="button" @click="CloseAlterPage">取消</button>
          <button type="button" @click="SubmitEditRoleMsg">确认</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async SubmitAllDistributionData() {
      ...
      this.SubmitRightMsg.authId = [
        ...this.HasSelectMsg.UserRightMsg,
        ...this.HasSelectMsg.DebtRightMsg,
        ...this.HasSelectMsg.AssetsRightMsgList,
      ].join(',')
